...
const fetchData = async () => {
  try {
    ...
    if (roomReservations.value.message !== undefined) {
      if (
        roomReservations.value.message === 'Room reservation does not exist or could not be find.'
      ) {
...
async function isRoomAvailableAtSpecificHour(roomReservations, specificDate, specificTime) {
  ...
  const reservationsInRange = roomReservations.value.filter((reservation) => {
    const reservationTimeStartRange = new Date(
      reservation.date.split('T')[0] + 'T' + reservation.time + 'Z'
    ).getTime()
    const reservationTimeEndRange =
      new Date(reservation.date.split('T')[0] + 'T' + reservation.time + 'Z').getTime() +
      1 * 60 * 60 * 1000
    return (
      (reservationTimeStartRange >= startOfRange && reservationTimeStartRange <= endOfRange) ||
      (reservationTimeEndRange >= startOfRange && reservationTimeEndRange <= endOfRange)
    )
  })
            ...
            <ul v-if="roomReservations.length !== 0">
              ...
              >
                ...
                <span
                  >Reserved by:
                  {{
                    username[roomReservation.userId]
                      ? username[roomReservation.userId].name !== undefined
                        ? username[roomReservation.userId].name
                        : 'N/A'
                      : 'N/A'
                  }}</span
                >
